...
INT32 HTTPParseCommandLineArgs(UINT32 argc, CHAR *argv[],HTTPParameters *pClientParams)
{
    ...
    for(nArg = 1;nArg < argc; nArg++)
    {
        ...
        if(memicmp(argv[nArg],"/?",2) == 0)
        {
            HTTPDumpHelp(NULL);
            return -1;
        }

        if(memicmp(argv[nArg],"/V",2) == 0)
        {
