...
UINT CNetworksDlg::NbtstatThread(LPVOID param)
{
	...
	if (pDlg->bRef == true)
	{
		...
		iCount.Format(_T("%d"), pDlg->m_BTC1T.m_List_BTC1T.GetItemCount());
		itemName = "BTC1T: " + iCount;
		pDlg->m_ListBox.InsertString(0, itemName);
		iCount.Format(_T("%d"), pDlg->m_LTC30M.m_List_LTC30M.GetItemCount());
		itemName = "LTC30M: " + iCount;
		pDlg->m_ListBox.InsertString(0, itemName);
		iCount.Format(_T("%d"), pDlg->m_LTC60M.m_List_LTC60M.GetItemCount());
		itemName = "LTC60M: " + iCount;
		pDlg->m_ListBox.InsertString(0, itemName);
		iCount.Format(_T("%d"), pDlg->m_UnknowType.m_List_UnknowType.GetItemCount());
...
{
	CString strIP;
	CString command = "sudo reboot\r\n";

	for (int i =0 ; i< (pDlg->m_ListCtrl.GetItemCount()); i++)
	{
		strIP = pDlg->m_ListCtrl.GetItemText(i, 0);
...
void CNetworksDlg::MultiLkMachine_GetStatus(CString iDestAddr, int ret, CString strHtmlData)
{
	...
	case 3:
		{
			...
			if (atoi(BA1) < 13 || atoi(BA1) > 15)	
				m_LTC60M.m_List_LTC60M.SetItemColor(m_LTC60M.m_line, 1, RGB(255,0,0), RGB(255,255,255));
			if (atoi(BA2) < 13 || atoi(BA2) > 15)	
				m_LTC60M.m_List_LTC60M.SetItemColor(m_LTC60M.m_line, 2, RGB(255,0,0), RGB(255,255,255));
			if (atoi(BA3) < 13 || atoi(BA3) > 15)	
				m_LTC60M.m_List_LTC60M.SetItemColor(m_LTC60M.m_line, 3, RGB(255,0,0), RGB(255,255,255));
			if (atoi(BA4) < 13 || atoi(BA4) > 15)	
				m_LTC60M.m_List_LTC60M.SetItemColor(m_LTC60M.m_line, 4, RGB(255,0,0), RGB(255,255,255));	
			if (SUM == "0" || atoi(SUM) > 60 || atoi(SUM) < 55)
	...
	case 1:
		{
			...
			m_BTC1T.m_List_BTC1T.AddItem(iDestAddr, BA1, BA2, BA3, BA4, SUM, poolUser1, pool1Status,  \
				poolUser2, pool2Status, poolUser3, pool3Status, RunTime, strTemp[0], strTemp[1],strTemp[2],strTemp[3]);
			if (atoi(BA1) < 240 || atoi(BA1) > 270)	
				m_BTC1T.m_List_BTC1T.SetItemColor(m_BTC1T.m_line, 1, RGB(255,0,0), RGB(255,255,255));
			if (atoi(BA2) < 240 || atoi(BA2) > 270)	
				m_BTC1T.m_List_BTC1T.SetItemColor(m_BTC1T.m_line, 2, RGB(255,0,0), RGB(255,255,255));
			if (atoi(BA3) < 240 || atoi(BA3) > 270)	
				m_BTC1T.m_List_BTC1T.SetItemColor(m_BTC1T.m_line, 3, RGB(255,0,0), RGB(255,255,255));
			if (atoi(BA4) < 240 || atoi(BA4) > 270)	
				m_BTC1T.m_List_BTC1T.SetItemColor(m_BTC1T.m_line, 4, RGB(255,0,0), RGB(255,255,255));	
			if (SUM == "0" || atoi(SUM) > 1100 || atoi(SUM) < 900)
				m_BTC1T.m_List_BTC1T.SetItemColor(m_BTC1T.m_line, 5, RGB(255,0,0), RGB(255,255,255));
	...
	case 2:
		...
		if (atoi(ICA1) < 7 || atoi(ICA1) > 8)	
			m_LTC30M.m_List_LTC30M.SetItemColor(m_LTC30M.m_line, 1, RGB(255,0,0), RGB(255,255,255));
		if (atoi(ICA2) < 7 || atoi(ICA2) > 8)	
			m_LTC30M.m_List_LTC30M.SetItemColor(m_LTC30M.m_line, 2, RGB(255,0,0), RGB(255,255,255));
		if (atoi(ICA3) < 7 || atoi(ICA3) > 8)	
			m_LTC30M.m_List_LTC30M.SetItemColor(m_LTC30M.m_line, 3, RGB(255,0,0), RGB(255,255,255));
		if (atoi(ICA4) < 7 || atoi(ICA4) > 8)	
			m_LTC30M.m_List_LTC30M.SetItemColor(m_LTC30M.m_line, 4, RGB(255,0,0), RGB(255,255,255));	
		if (SUM == "0" || atoi(SUM) > 30 || atoi(SUM) < 28)
			m_LTC30M.m_List_LTC30M.SetItemColor(m_LTC30M.m_line, 5, RGB(255,0,0), RGB(255,255,255));
...
void CNetworksDlg::OnButton3() 
{
// 	//SetItemText
// 	LPTSTR pszText = "ste";
// 	//m_MainTab.GetItem(0,&mItem);
// 	TC_ITEM ti;
//     ti.mask = TCIF_TEXT;
//     ti.pszText = pszText;
// 	m_MainTab.GetItem(2,&ti); 
//     VERIFY (m_MainTab.SetItem (2, &ti));
// 	Invalidate();
	countMachine();
}
...
void CNetworksDlg::countMachine()
{
	...
	itemName = "BTC1T(" +iCount+ ")";
	//pDlg->m_ListBox.InsertString(0, itemName);
	LPTSTR pszText = (LPTSTR)(LPCTSTR)itemName;
	ti.mask = TCIF_TEXT;
	ti.pszText = pszText;
	...
	itemName = "LTC30M(" +iCount+")";
	pszText = (LPTSTR)(LPCTSTR)itemName;
	ti.mask = TCIF_TEXT;
	ti.pszText = pszText;
	m_MainTab.GetItem(1,&ti); 
	pDlg->m_MainTab.SetItem (1, &ti);
	...
	pDlg->m_MainTab.SetItem (3, &ti);
	
	this->UpdateWindow();
	this->Invalidate();
	//CNetworksDlg::Invalidate();
}