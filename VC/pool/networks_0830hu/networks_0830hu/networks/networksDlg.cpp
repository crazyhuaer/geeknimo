...
BOOL CNetworksDlg::OnInitDialog()
{
	...
	m_MainTab.InsertItem(1, _T("BTC15T"));
	m_MainTab.InsertItem(2,_T("LTC30M"));
	m_MainTab.InsertItem(3,_T("SZLTC30M"));
	m_MainTab.InsertItem(4,_T("LTC60M"));
	m_MainTab.InsertItem(5,_T("UNKNOWTYPE"));
	m_BTC1T.Create(IDD_DIALOG_BTC1T,GetDlgItem(IDC_TAB1));
	m_BTC15T.Create(IDD_DIALOG_BTC15T,GetDlgItem(IDC_TAB1));
	m_LTC30M.Create(IDD_DIALOG_LTC30M,GetDlgItem(IDC_TAB1));
	m_SZLTC30M.Create(IDD_DIALOG_SZLTC30M, GetDlgItem(IDC_TAB1));
	...
	m_BTC15T.m_List_BTC15T.SetWindowPos(NULL,0,0,1200,360,SWP_NOMOVE);
	m_LTC30M.m_List_LTC30M.SetWindowPos(NULL,0,0,1200,360,SWP_NOMOVE);
	m_SZLTC30M.m_List_szLTC30M.SetWindowPos(NULL,0,0,1200,360,SWP_NOMOVE);
	...
	m_BTC15T.ShowWindow(0);
	m_LTC30M.ShowWindow(0);
	m_SZLTC30M.ShowWindow(0);
	...
	m_BTC15T.m_List_BTC15T.SetExtendedStyle(LVS_EX_FULLROWSELECT|LVS_EX_GRIDLINES|LVS_EX_FULLROWSELECT);
	m_BTC15T.m_List_BTC15T.SetHeadings(_T("IP,90;BA1,50;BA2,50;BA3,50;BA4,50;BA5,50;BA6,50;SUM,40;PL1-USER,100;PL1_STATUS,75;PL2-USER,100;PL2-STATUS,75;PL3-USER,100;PL3-STATUS,75;RUN-TIME,90;BA10-TEMP,75;BA11-TEMP,75;BA12-TEMP,75;BA13-TEMP,75;BA14-TEMP,75;BA15-TEMP,75"));
	...
	m_SZLTC30M.m_List_szLTC30M.SetExtendedStyle(LVS_EX_FULLROWSELECT|LVS_EX_GRIDLINES|LVS_EX_FULLROWSELECT);
	m_SZLTC30M.m_List_szLTC30M.SetHeadings(_T("IP,90;ICA1,50;ICA2,50;ICA3,50;ICA4,50;ICA5,50;ICA6,50;ICA7,50;ICA8,50; \
			ICA9,50;ICA10,50;ICA11,50;ICA12,50;SUM,50; PL1-USER,100; PL1_STATUS,75; PL2-USER,100; PL-STATUS,75; PL-USER, 100;PL_STATUS,75; RunTime,70"));
	...
	//m_BTC1T.m_List_BTC1T.AddItem(_T("192.168.1.53"), _T("0"), _T("0"), _T("0"), _T("0"), _T("0"), _T("unknown type"), _T("0"), _T("unknown type"),  \
...
void CNetworksDlg::countMachine()
{
	...
	iCount.Format(_T("%d"), m_BTC15T.m_List_BTC15T.GetItemCount());
	itemName = "BTC15T(" +iCount+ ")";
	//pDlg->m_ListBox.InsertString(0, itemName);
	...
	iCount.Format(_T("%d"), m_SZLTC30M.m_List_szLTC30M.GetItemCount());
	itemName = "SZLTC30M(" +iCount+")";
	...
	m_MainTab.GetItem(4,&ti); 
	ti.pszText = (LPTSTR)(LPCTSTR)itemName;
	ti.mask = TCIF_TEXT;
	m_MainTab.SetItem(4, &ti);
	
	iCount.Format(_T("%d"), m_UnknowType.m_List_UnknowType.GetItemCount());
	itemName = "UNKNOWTYPE(" +iCount+")";
	m_MainTab.GetItem(5,&ti); 
	ti.pszText = (LPTSTR)(LPCTSTR)itemName;
	ti.mask = TCIF_TEXT;	
	m_MainTab.SetItem (5, &ti);

	//////////////////////////////////////////////////////////////////////////
	m_ListBox.InsertString(0,"======================");
	m_ListBox.InsertString(0,"不良品:"+iCount+"台");
	iCount.Format(_T("%d"), m_LTC60M.m_List_LTC60M.GetItemCount());
	m_ListBox.InsertString(0,"LTC 60M:"+iCount+"台");
	iCount.Format(_T("%d"), m_SZLTC30M.m_List_szLTC30M.GetItemCount());
	m_ListBox.InsertString(0,"LTC 30M(SZ):"+iCount+"台");
	iCount.Format(_T("%d"), m_LTC30M.m_List_LTC30M.GetItemCount());
	m_ListBox.InsertString(0,"LTC 30M:"+iCount+"台");
	iCount.Format(_T("%d"), m_BTC1T.m_List_BTC1T.GetItemCount());
	m_ListBox.InsertString(0,"BTC 1T:"+iCount+"台");
	iCount.Format(_T("%d"), m_BTC15T.m_List_BTC15T.GetItemCount());
	m_ListBox.InsertString(0,"BTC 1.5T:"+iCount+"台");
	
	int sum = m_UnknowType.m_List_UnknowType.GetItemCount() + m_LTC60M.m_List_LTC60M.GetItemCount() + 
				m_SZLTC30M.m_List_szLTC30M.GetItemCount() + m_LTC30M.m_List_LTC30M.GetItemCount() +
				m_BTC1T.m_List_BTC1T.GetItemCount() + m_BTC15T.m_List_BTC15T.GetItemCount();
	
	if (sum == 0)
		iCount.Format(_T("%f"),"0");
	else
		iCount.Format(_T("%f"),m_UnknowType.m_List_UnknowType.GetItemCount()*1.0/sum*100);

	m_ListBox.InsertString(0,"不良品率:"+iCount+"%");

	iCount.Format(_T("%d"), sum);	
	m_ListBox.InsertString(0,"总机器数:"+iCount+"台");
...
void CNetworksDlg::sshSrv(CString command, CString hostname, int port)
{
    ...
    if (success != true) {
		...
		m_ListBox.InsertString(0, strOut);
        return;
    }
...
{
	char *strIP = (char*)param;
	ShellExecute(NULL,"open","REBOOT.EXE",strIP,"",SW_HIDE);
	/*char pBuf[MAX_PATH];
	...
	CString tempCmd = "\"" + strCurPath  + "\\reboot.exe" + "\" "  + (*strIP);
...
UINT CNetworksDlg::MultiRebootThread(LPVOID param)
{
	...
	{
		for (int i =0 ; i< (pDlg->m_SZLTC30M.m_List_szLTC30M.GetItemCount()); i++)
		{
			strIP = pDlg->m_SZLTC30M.m_List_szLTC30M.GetItemText(i, 0);
...
void CNetworksDlg::OnSingleReboot() 
{
	...
	if (tab == 0)
	{
		...
		//得到选中项的内容（0表示第一列，同理1,2,3...表示第二，三，四...列）
		strIP=pDlg->m_BTC1T.m_List_BTC1T.GetItemText(listIndex,0);
	}
	else if(tab == 1)
	{
		//BTC1.5T
		//首先得到点击的位置 
		pos=pDlg->m_BTC15T.m_List_BTC15T.GetFirstSelectedItemPosition();
		if(pos==NULL) 
			return; 
		
		//得索引，通过POSITION转化 
		listIndex=(int)pDlg->m_BTC15T.m_List_BTC15T.GetNextSelectedItem(pos); 
		
		//得到选中项的内容（0表示第一列，同理1,2,3...表示第二，三，四...列）
		strIP=pDlg->m_BTC15T.m_List_BTC15T.GetItemText(listIndex,0);
	}
	else if (tab == 2)
	{
		//LTC 30M
		...
		//得到选中项的内容（0表示第一列，同理1,2,3...表示第二，三，四...列）
		strIP=pDlg->m_LTC30M.m_List_LTC30M.GetItemText(listIndex,0);
	}
	else if (tab == 3)
	{
		//SZ LTC 30M
		pos=pDlg->m_SZLTC30M.m_List_szLTC30M.GetFirstSelectedItemPosition();
		if(pos==NULL) 
			return; 
		
		//得索引，通过POSITION转化 
		listIndex=(int)pDlg->m_SZLTC30M.m_List_szLTC30M.GetNextSelectedItem(pos); 
		strIP=pDlg->m_SZLTC30M.m_List_szLTC30M.GetItemText(listIndex,0);
	}
	else if (tab == 4)
	{
		...
		strIP=pDlg->m_LTC60M.m_List_LTC60M.GetItemText(listIndex,0);
	}
	else if (tab == 5)
	{
		...
		//得到选中项的内容（0表示第一列，同理1,2,3...表示第二，三，四...列）
		strIP=pDlg->m_UnknowType.m_List_UnknowType.GetItemText(listIndex,0);
	}
	//AfxBeginThread(SingleRebootThread,(LPVOID)(&strIP),CREATE_NEW);
	char* sz = strIP.GetBuffer(strIP.GetLength());
	ShellExecute(NULL,"open","REBOOT.EXE",sz,"",SW_HIDE);
	m_ListBox.InsertString(0, strIP + " reboot");
...
void CNetworksDlg::OnMutipleReboot() 
{
	...
	for (int i =0 ; i< (pDlg->m_UnknowType.m_List_UnknowType.GetItemCount()); i++)
	{
		strIP = pDlg->m_UnknowType.m_List_UnknowType.GetItemText(i, 0);
		char *sz = strIP.GetBuffer(strIP.GetLength());
		AfxBeginThread(SingleRebootThread,(LPVOID)sz,CREATE_NEW);
	}
	//GetDlgItem(IDC_BUTTON_REFRESH)->EnableWindow(FALSE);
	//GetDlgItem(IDC_REFRESH)->EnableWindow(FALSE);
	//GetDlgItem(IDC_SINGLE_REBOOT)->EnableWindow(FALSE);
...
void CNetworksDlg::MultiLkMachine_GetStatus(CString iDestAddr, int ret, CString strHtmlData)
{
	...
	//////////////////////////////////////////////////////////////////////////
	/*	if (atoi(BA1) < 247 || atoi(BA1) > 267 || atoi(BA2) < 247 || atoi(BA2) > 267 \
			|| atoi(BA3) < 247 || atoi(BA3) > 267 || atoi(BA4) < 247 || atoi(BA4) > 267 \
			|| atoi(BA5) < 247 || atoi(BA6) > 267 || atoi(BA6) < 247 || atoi(BA6) > 267 \
			|| atoi(SUM) < 1480 || atoi(SUM) > 1600)
		{
			m_UnknowType.m_List_UnknowType.AddItem(iDestAddr, BA1, BA2, BA3, BA4, BA5, BA6, SUM, poolUser1, pool1Status,  \
				poolUser2, pool2Status, poolUser3, pool3Status, RunTime, strTemp[0], strTemp[1],strTemp[2],strTemp[3], strTemp[4],strTemp[5]);
			if (atoi(BA1) < 247 || atoi(BA1) > 267)	
				m_UnknowType.m_List_UnknowType.SetItemColor(m_UnknowType.m_line, 1, RGB(255,0,0), RGB(255,255,255));
			if (atoi(BA2) < 247 || atoi(BA2) > 267)	
				m_UnknowType.m_List_UnknowType.SetItemColor(m_UnknowType.m_line, 2, RGB(255,0,0), RGB(255,255,255));
			if (atoi(BA3) < 247 || atoi(BA3) > 267)	
				m_UnknowType.m_List_UnknowType.SetItemColor(m_UnknowType.m_line, 3, RGB(255,0,0), RGB(255,255,255));
			if (atoi(BA4) < 247 || atoi(BA4) > 267)	
				m_UnknowType.m_List_UnknowType.SetItemColor(m_UnknowType.m_line, 4, RGB(255,0,0), RGB(255,255,255));
			if (atoi(BA5) < 247 || atoi(BA5) > 267)	
				m_UnknowType.m_List_UnknowType.SetItemColor(m_UnknowType.m_line, 5, RGB(255,0,0), RGB(255,255,255));
			if (atoi(BA6) < 247 || atoi(BA6) > 267)	
				m_UnknowType.m_List_UnknowType.SetItemColor(m_UnknowType.m_line, 6, RGB(255,0,0), RGB(255,255,255));
			if (SUM == "0" || atoi(SUM) > 1600 || atoi(SUM) < 1480)
				m_UnknowType.m_List_UnknowType.SetItemColor(m_UnknowType.m_line, 7, RGB(255,0,0), RGB(255,255,255));
			m_UnknowType.m_line=m_UnknowType.m_line+1;
		}*/
		m_BTC15T.m_List_BTC15T.AddItem(iDestAddr, BA1, BA2, BA3, BA4,BA5,BA6, SUM, poolUser1, pool1Status,  \
			poolUser2, pool2Status, poolUser3, pool3Status, RunTime, strTemp[0], strTemp[1],strTemp[2],strTemp[3],strTemp[4],strTemp[5]);
		if (atoi(BA1) < 247 || atoi(BA1) > 267)
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 1, RGB(255,0,0), RGB(255,255,255));
		if (atoi(BA2) < 247 || atoi(BA2) > 267)
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 2, RGB(255,0,0), RGB(255,255,255));
		if (atoi(BA3) < 247 || atoi(BA3) > 267)
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 3, RGB(255,0,0), RGB(255,255,255));
		if (atoi(BA4) < 247 || atoi(BA4) > 267)
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 4, RGB(255,0,0), RGB(255,255,255));
		if (atoi(BA5) < 247 || atoi(BA5) > 267)
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 5, RGB(255,0,0), RGB(255,255,255));
		if (atoi(BA6) < 247 || atoi(BA6) > 267)
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 6, RGB(255,0,0), RGB(255,255,255));
		if (SUM == "0" || atoi(SUM) > 1600 || atoi(SUM) < 1480)
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 7, RGB(255,0,0), RGB(255,255,255));
		
		if (pool1Status != "Alive")
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 9, RGB(255,0,0), RGB(255,255,255));
		if (pool2Status != "Alive")
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 11, RGB(255,0,0), RGB(255,255,255));
		if (pool3Status != "Alive")
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 13, RGB(255,0,0), RGB(255,255,255));
		
		
		if (strTemp[0] > 70 || strTemp[0] == "0")
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 15, RGB(255,0,0), RGB(255,255,255));
		if (strTemp[1] > 70 || strTemp[1] == "0")
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 16, RGB(255,0,0), RGB(255,255,255));
		if (strTemp[2] > 70 || strTemp[2] == "0")
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 17, RGB(255,0,0), RGB(255,255,255));
		if (strTemp[3] > 70 || strTemp[3] == "0")
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 18, RGB(255,0,0), RGB(255,255,255));
		if (strTemp[4] > 70 || strTemp[4] == "0")
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 19, RGB(255,0,0), RGB(255,255,255));
		if (strTemp[5] > 70 || strTemp[5] == "0")
			m_BTC15T.m_List_BTC15T.SetItemColor(m_BTC15T.m_line, 20, RGB(255,0,0), RGB(255,255,255));
		m_BTC15T.m_line=m_BTC15T.m_line + 1;
	}
	break;
	//SZ LTC 30
	case 4:
		...
		/*if ((atof(ICA[0]) > 2.5) || (atof(ICA[0]) < 2.6) || (atof(ICA[1]) > 2.5) || (atof(ICA[1]) > 2.6) ||  \
			(atof(ICA[2]) > 2.5) || (atof(ICA[2]) < 2.6) || (atof(ICA[3]) > 2.5) || (atof(ICA[3]) > 2.6) ||  \
			(atof(ICA[4]) > 2.5) || (atof(ICA[4]) < 2.6) || (atof(ICA[5]) > 2.5) || (atof(ICA[5]) > 2.6) ||  \
			(atof(ICA[6]) > 2.5) || (atof(ICA[6]) < 2.6) || (atof(ICA[7]) > 2.5) || (atof(ICA[7]) > 2.6) ||  \
			(atof(ICA[8]) > 2.5) || (atof(ICA[8]) < 2.6) || (atof(ICA[9]) > 2.5) || (atof(ICA[9]) > 2.6) ||  \
			(atof(ICA[10]) > 2.5) || (atof(ICA[10]) < 2.6) || (atof(ICA[11]) > 2.5) || (atof(ICA[11]) > 2.6))*/
		{
			m_SZLTC30M.m_List_szLTC30M.AddItem(iDestAddr,ICA[0],ICA[1],ICA[2],ICA[3],ICA[4],ICA[5],ICA[6],ICA[7], \
				ICA[8],ICA[9],ICA[10],ICA[11],SUM, poolUser1, pool1Status, poolUser2, pool2Status, poolUser3, pool3Status, RunTime);
			if ((atof(ICA[0]) > 2.8) || (atof(ICA[0]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 1, RGB(255,0,0), RGB(255,255,255));
			if ((atof(ICA[1]) > 2.8) || (atof(ICA[1]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 2, RGB(255,0,0), RGB(255,255,255));
			if ((atof(ICA[2]) > 2.8) || (atof(ICA[2]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 3, RGB(255,0,0), RGB(255,255,255));
			if ((atof(ICA[3]) > 2.8) || (atof(ICA[3]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 4, RGB(255,0,0), RGB(255,255,255));
			if ((atof(ICA[4]) > 2.8) || (atof(ICA[4]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 5, RGB(255,0,0), RGB(255,255,255));
			if ((atof(ICA[5]) > 2.8) || (atof(ICA[5]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 6, RGB(255,0,0), RGB(255,255,255));
			if ((atof(ICA[6]) > 2.8) || (atof(ICA[6]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 7, RGB(255,0,0), RGB(255,255,255));
			if ((atof(ICA[7]) > 2.8) || (atof(ICA[7]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 8, RGB(255,0,0), RGB(255,255,255));
			if ((atof(ICA[8]) > 2.8) || (atof(ICA[8]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 9, RGB(255,0,0), RGB(255,255,255));
			if ((atof(ICA[9]) > 2.8) || (atof(ICA[9]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 10, RGB(255,0,0), RGB(255,255,255));
			if ((atof(ICA[10]) > 2.8) || (atof(ICA[10]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 11, RGB(255,0,0), RGB(255,255,255));
			if ((atof(ICA[11]) > 2.8) || (atof(ICA[11]) < 2.4))
				m_SZLTC30M.m_List_szLTC30M.SetItemColor(m_SZLTC30M.m_line, 12, RGB(255,0,0), RGB(255,255,255));
	...
	case 3:
		{
				...
				|| atoi(BA3) < 13.75 || atoi(BA3) > 16.00 || atoi(BA4) < 13.75 || atoi(BA4) > 16.00)
			{
				...
				if (SUM == "0" || atof(SUM) > 62.00|| atof(SUM) < 55.00)
					m_UnknowType.m_List_UnknowType.SetItemColor(m_LTC60M.m_line, 5, RGB(255,0,0), RGB(255,255,255));
				m_UnknowType.m_line=m_UnknowType.m_line + 1;
			}
			else
			{
...
OUTP:	default:
		{
			m_UnknowType.m_List_UnknowType.AddItem(iDestAddr, _T("0"), _T("0"), _T("0"), _T("0"), _T("0"), _T("0"),_T("0"),  \
