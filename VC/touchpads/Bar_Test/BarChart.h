...
public:
	...
	BOOL ReadFromDatabase(CString szDSN, CString szSPName, CString szParameterList = _T(""),
					COLORREF colorBars = RGB(0, 0, 0),
					CString szUsername = _T(""), CString szPass = _T(""));
	...
	BOOL SaveToFile(CString szPath = _T(""));

	void SetGridLines(int nHorLineCount, int nVerLineCount, BOOL bFixedSize = FALSE, int nFixedSize = 0);

	BOOL Create(const RECT& rect, CWnd* pParentWnd, UINT nControlID, CString szChartTitle=_T(""));
	virtual ~CBarChart();
