...
BOOL CNetworksDlg::OnInitDialog()
{
	...
	m_MainTab.InsertItem(2,_T("SZLTC30M"));
	m_MainTab.InsertItem(3,_T("LTC60M"));
	m_MainTab.InsertItem(4,_T("UNKNOWTYPE"));
...
void CNetworksDlg::countMachine()
{
	...
	int sum = m_UnknowType.m_List_UnknowType.GetItemCount() + m_LTC60M.m_List_LTC60M.GetItemCount() + 
				...
				m_BTC1T.m_List_BTC1T.GetItemCount();
...
UINT CNetworksDlg::SingleRebootThread(LPVOID param)
{
	...
	else if (tab == 2)
	{
		...
		//得到选中项的内容（0表示第一列，同理1,2,3...表示第二，三，四...列）
		CString strIP=pDlg->m_SZLTC30M.m_List_szLTC30M.GetItemText(listIndex,0);
		command = "sudo reboot\r\n";
		//command = "sudo pkill cgminer\r\n";
		//command +="cd /var/config\r\n";
		//command += "sudo ./run.sh\r\n";
