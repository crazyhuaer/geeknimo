...
void CBarChart::CChartBar::CreateLabelFont()
{
	...
	_tcscpy(m_lf.lfFaceName,_T("Tahoma"));
	m_lf.lfHeight=-10;
	m_lf.lfWidth=0;
	m_lf.lfEscapement = 0;
	...
	for (int i = 0; i<m_chartData.GetCount();i++)
	{
		...
		if (m_bShowBarText || m_bShowLabel)
		{
			...
			if (m_bShowBarText)
			{
				...
				if (m_nBarTextType== 0 )
				{
					str.Format(_T("%0.01f%%"), (m_chartData.GetValue(i)*100)/m_chartData.GetMaxValue());
				...
				{
					str.Format(_T("%0.02f"), m_chartData.GetValue(i));
					pDC->DrawText(str, CRect(x, y - BAR_TXT_PERCENT_HEIGHT, x+ m_nBarWidth+ m_nBarGap-1, y),
						DT_LEFT | DT_SINGLELINE | DT_VCENTER);
				}
			}
			...
			ULONG ulODBCVer)
{
	...
	return 
		OpenTable(szDSN, szStoredProc, _T(""), _T(""), szParameterList, 
...
BOOL CBarChart::CChartDatabase::Connect(CString szDSN, CString szUsername, CString szPass)
{
	...
	m_sqlRet = SQLConnect(m_hDbConn, (_TUCHAR *)(LPCTSTR)szDSN, SQL_NTS,
		(_TUCHAR *)(LPCTSTR)szUsername, SQL_NTS,
		(_TUCHAR *)(LPCTSTR)szPass, SQL_NTS);
...
{
	CString	szQuery;
	szQuery.Format(_T("SELECT %s, %s FROM %s %s"), 
...
BOOL CBarChart::CChartDatabase::ExecuteSQL(CString& szQuery)
{
    ...
    m_sqlRet = SQLExecDirect(m_hstmt, (_TUCHAR*)(LPCTSTR)szQuery, SQL_NTS);
    if(m_sqlRet != SQL_SUCCESS && m_sqlRet != SQL_SUCCESS_WITH_INFO)
	{
